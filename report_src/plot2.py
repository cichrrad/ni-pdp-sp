...
impls = ["omp_data", "omp_task", "mpi_1", "mpi_2", "mpi_3"]
...
sns.barplot(data=melted, x="file", y="speedup", hue="implementation")
plt.title("Speedup oproti sekvenční verzi (moje CPU)")
...
sns.barplot(data=combined, x="file", y="speedup_omp_data_vs_task")
...
for impl in impls:
    ...
    if speedup_col in combined.columns:
        sns.barplot(data=combined, x="file", y=speedup_col)
